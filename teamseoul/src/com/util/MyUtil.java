...
public class MyUtil {
    ...
    //********************************************
  	// javascript 페이지 처리(javascript 지정 함수 호출, methodName:호출할 스크립트 함수명,areaCode:지역코드 )
    public String pagingMethod(int current_page, int total_page, String methodName, String code) {
		if(current_page < 1 || total_page < 1)
			return "";
        ...
        n = current_page - numPerBlock;
        if ((total_page > numPerBlock) && (currentPageSetUp > 0)) {
			sb.append("<a onclick='"+methodName+"(1,"+code+");'>처음</a>");
			sb.append("<a onclick='"+methodName+"("+n+","+code+");'>이전</a>");
        }
        ...
        while((page <= total_page) && (page <= currentPageSetUp + numPerBlock)) {
           if(page == current_page) {
			   ...
			   sb.append("<a onclick='"+methodName+"("+page+","+code+");' class='numBox'>"+page+"</a>");
           }
           page++;
        }
		...
		if(n>total_page) n=total_page;
        if (total_page - currentPageSetUp > numPerBlock) {
			sb.append("<a onclick='"+methodName+"("+n+","+code+");'>다음</a>");
			sb.append("<a onclick='"+methodName+"("+total_page+","+code+");'>끝</a>");
        }
		sb.append("</div>");
